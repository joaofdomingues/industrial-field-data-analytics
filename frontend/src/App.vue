<<template>
  ...
  <div class="panel chart-panel">
    <div class="panel-header">
      <div>
        <h2>PySpark Load Summary</h2>
        <p>Processed machine load analytics generated by the PySpark pipeline</p>
      </div>
      <span class="chip">PySpark</span>
    </div>

    <Bar :data="pysparkLoadChartData" :options="barChartOptions" />
  </div>

  <div class="panel chart-panel">
    <div class="panel-header">
      <div>
        <h2>PySpark Reliability Scores</h2>
        <p>Reliability scoring calculated from processed telemetry outputs</p>
      </div>
      <span class="chip">Databricks-style</span>
    </div>

    <Bar :data="pysparkReliabilityChartData" :options="barChartOptions" />
  </div>
</section>
...
<section class="panel">
  <div class="panel-header">
    <div>
      <h2>PySpark Detected Anomalies</h2>
      <p>Operational anomalies processed from the PySpark analytics layer</p>
    </div>
    <span class="chip">Batch Analytics</span>
  </div>
  ...
  <div class="table-wrapper">
    <table>
      ...
      <tbody>
        <tr v-for="item in pysparkAnomalies.slice(0, 10)" :key="item.machine_code + item.recorded_at">
          <td>
            <strong>{{ item.machine_name }}</strong>
            <small>{{ item.machine_code }}</small>
          </td>
        ...
        <tr v-if="!pysparkAnomalies.length">
          <td colspan="7" class="empty">No PySpark anomalies available.</td>
        </tr>
      </tbody>
    </table>
  </div>
...
data() {
  ...
  computed: {
    ...
    pysparkLoadChartData() {
  return {
    labels: this.pysparkLoadSummary.map(item => item.machine_name),
    datasets: [
      {
        label: 'Average Load %',
        data: this.pysparkLoadSummary.map(item => Number(item.avg_load || 0)),
...
pysparkReliabilityChartData() {
  return {
    labels: this.pysparkLoadSummary.map(item => item.machine_name),
    datasets: [
      {
        label: 'Reliability Score',
        data: this.pysparkLoadSummary.map(item => Number(item.avg_reliability_score || 0)),
        backgroundColor: 'rgba(34, 197, 94, 0.72)',
        borderColor: '#86efac',
        borderWidth: 1,
        borderRadius: 10
...
async getJson(path) {
  ...
  return
        ...
        await this.loadPySparkAnalytics()
      } catch (error) {
    ...
    async loadPySparkAnalytics() {
  try {
    const [loadSummary, anomalies, globalMetrics] = await Promise.all([
      this.getJson('/pyspark/load-summary/'),
      this.getJson('/pyspark/anomalies/'),
      this.getJson('/pyspark/global-metrics/')
    ])

    this.pysparkLoadSummary = loadSummary || []
    this.pysparkAnomalies = anomalies || []
    this.pysparkGlobalMetrics = globalMetrics || []
  } catch (error) {
    console.error('Failed to load PySpark analytics', error)
  }
},
